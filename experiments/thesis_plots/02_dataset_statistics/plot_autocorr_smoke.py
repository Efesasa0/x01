...
def parse_args():
    ...
    parser.add_argument("--out", type=Path, default=Path(__file__).with_name("autocorr_smoke.png"))
    return parser.parse_args()


def temporal_autocorr(frames):
    T = frames.shape[0]
...
def read_trajectory(data, trajectory_index, frame_step, spatial_stride):
    ...
    return trajectory[:, ::spatial_stride, ::spatial_stride]


def compute_curves(path, start_index, count, frame_step, spatial_stride):
    data = np.load(path, mmap_mode="r")
    stop_index = min(start_index + count, data.shape[0])
    curves = []
    for trajectory_index in range(start_index, stop_index):
        trajectory = read_trajectory(data, trajectory_index, frame_step, spatial_stride)
        curves.append(temporal_autocorr(trajectory))
...
def main():
    args = parse_args()
    old_t, old_curves = compute_curves(
        args.old_data,
        args.start_traj,
        args.count,
        args.frame_step,
    ...
    new_t, new_curves = compute_curves(
        args.new_data,
        args.start_traj,
        args.count,
        args.frame_step,
    ...
    ax.fill_between(old_t, old_mean - old_std, old_mean + old_std, color="0.75", alpha=args.old_alpha, linewidth=0)
    new_band = ax.fill_between(
        ...
        facecolor="white",
        edgecolor="0.45",
        alpha=args.new_alpha,
        linewidth=0.0,
    )
    new_band.set_hatch("///")
    ax.plot(old_t, old_mean, color="black", linestyle="-", linewidth=args.linewidth, label="Initial dataset")
    ax.plot(new_t, new_mean, color="black", linestyle=":", linewidth=args.linewidth, label="Generated dataset")
