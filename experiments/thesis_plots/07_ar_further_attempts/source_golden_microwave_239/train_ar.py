...
def make_loader(
    ...
    ds = KMFlowDataset(data_path, split=split, std=std, skip_n_frames=skip_n_frames, pick_n_trajs=pick_n_trajs)
    sampler = None
...
def main(cfg: DictConfig) -> None:
    ...
    val_T = val_ds.samples_per_traj
    gt_probe = torch.stack([val_ds[t][0] for t in range(val_T)] + [val_ds[val_T - 1][1]])
    ...
    for epoch in range(1, epochs + 1):
        ...
        totals = dict(loss=0.0, fwd=0.0, bwd=0.0, recon=0.0, ortho=0.0)  # fwd/bwd/recon/ortho only used for koopman
        n_t = 0
        ...
        for step, (x, y) in enumerate(train_loader, 1):
            ...
            optimizer.zero_grad()
            if cfg.ar.name == "koopman":
                loss, log = loss_koopman(model, x, y, lambda_ortho=cfg.ar.lambda_ortho, lambda_bwd=cfg.ar.lambda_bwd)
            else:
                pred = model(x)
        ...
        if rank0:
            model.eval()
            val_totals = dict(loss=0.0, fwd=0.0, bwd=0.0, recon=0.0, ortho=0.0)
            n_v = 0
            with torch.no_grad():
                for x, y in val_loader:
                    ...
                    if cfg.ar.name == "koopman":
                        _, vlog = loss_koopman(
                            base_model, x, y, lambda_ortho=cfg.ar.lambda_ortho, lambda_bwd=cfg.ar.lambda_bwd
                        )
