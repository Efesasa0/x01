...
def make_loader(
    ...
    return (
        DataLoader(
            ...
            pin_memory=True,
            drop_last=(split == "train" and sampler is not None),
        ),
        sampler,
    )
...
def main(cfg: DictConfig) -> None:
    ...
    pick_n_trajs = 2 if smoke else None
    data_downsample = 4 if smoke else cfg.data.downsample
    image_resolution = cfg.sr.image_resolution // data_downsample if smoke else cfg.sr.image_resolution
    ...
    if rank0:
        ...
        # Once-only contract dump: prove which keys this run will log so dashboard panels
        # can be debugged ("no data for selected runs" usually means a key mismatch).
        _wandb_keys = [
            "train/loss", "train/lr",
            "epoch/train_loss", "epoch/val_loss", "epoch",
            "val/samples", "val/energy_spectrum",
            "val/sample_mse_base", "val/sample_mse_ema",
            "test/loss", "test/samples", "test/energy_spectrum",
        ]
        print(f"wandb run url : {wandb.run.url if wandb.run else 'n/a'}", flush=True)
        print(f"wandb keys    : {_wandb_keys}", flush=True)
        print(f"viz.enabled={cfg.viz.enabled}  viz.every={cfg.viz.every}", flush=True)
    ...
    for epoch in range(1, epochs + 1):
        ...
        if rank0:
            ...
            viz_due = cfg.viz.enabled and (epoch % cfg.viz.every == 0)
            print(
                f"  viz check: enabled={cfg.viz.enabled} every={cfg.viz.every} "
                f"epoch={epoch} -> {'FIRING' if viz_due else 'skip'}",
                flush=True,
            )
            if viz_due:
                log_samples(synthesize_sample_for_viz, val_ds, device, epoch, train_std, tag="val")
                log_energy_spectrum(synthesize_sample_for_viz, gt_probe, device, epoch, tag="val")
                log_ema_vs_base_mse(gt_probe, epoch, tag="val")
                print(
                    "  viz logged keys: val/samples, val/energy_spectrum, "
                    "val/sample_mse_base, val/sample_mse_ema",
                    flush=True,
                )
            ...
            if epoch % cfg.training.ckpt_every == 0:
                ...
                path = f"{cfg.training.ckpt_dir}/{cfg.sr.name}_sr_epoch{epoch:04d}.pt"
    ...
    if not smoke and rank0:
        ...
        if cfg.viz.enabled:
            ...
            log_samples(synthesize_sample_for_viz, test_ds, device, epochs, train_std, tag="test")
            log_energy_spectrum(synthesize_sample_for_viz, gt_test_probe, device, epochs, tag="test")
