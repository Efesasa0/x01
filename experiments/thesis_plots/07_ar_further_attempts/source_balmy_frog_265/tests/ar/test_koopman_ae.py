...
def test_loss_keys(model, xy):
    ...
    assert set(log.keys()) == {"loss", "forward_loss", "backward_loss", "reconstruction_loss", "ortho_loss"}
...
def test_loss_equals_sum(model, xy):
    ...
    lambda_ortho = 1e-3
    lambda_bwd = 0.1
    loss, log = loss_koopman(model, x, y, lambda_ortho=lambda_ortho)
    expected = (
        log["forward_loss"]
        + lambda_bwd * log["backward_loss"]
        + log["reconstruction_loss"]
        + lambda_ortho * log["ortho_loss"]
    )
